...
FRESULT dir_find (  /* FR_OK(0):succeeded, !=0:error */
    FDIR* dp        /* Pointer to the directory object linked to the file name */
)
{
    FRESULT res;
    BYTE c, *dir;
...
#endif
                    {
                        mem_cpy(&sdj, &dj, sizeof (FDIR));   /* Open the sub-directory */
                        sdj.sclust = dclst;
                        res = dir_sdi(&sdj, 2);
                        if (res == FR_OK) {
...
FRESULT f_mkfs (
    ...
    UINT start,         /* Partition start when FDISK partitioning rule is used (in sectors) */
    UINT end            /* Partition end when FDISK partitioning rule is used (in sectors) */
)
{
    ...
    if (_MULTI_PARTITION && part) {
        ...
        if (sfd) {  /* No partition table (SFD) */
            ...
            ST_DWORD(tbl + 8, b_vol);       /* Partition start in LBA */
